...
class DBHandler:
    def __init__(self):
        ...
        self.connection = pymysql.connect(host=credentials.DB['host'],
    ...
    def get_user_id(self, wikiloc_user_id, wikiloc_user_name):
        """
        ...
        """
        user_id = self.check_user_exists(wikiloc_user_id)
        if user_id is not -1:
            return user_id
        else:
            self.create_user(wikiloc_user_id, wikiloc_user_name)
            return self.check_user_exists(wikiloc_user_id)

    def create_user(self, wikiloc_user_id, wikiloc_user_name):
        """
    ...
    def check_user_exists(self, wikiloc_user_id):
        """
    ...
    def build_insert_command(self, trail_data, category_id, user_id):
        """
    ...
    def insert_into_db(self, trails_data):
        """
        ...
        try:
            with self.connection.cursor() as cursor:
                ...
                for trail_data in trails_data:
                    ...
                    command = self.build_insert_command(trail_data, category_id, user_id)
                    ...
                    try:
                        ...
                        print(f'Failed executing command: {command}\n Exception string:{e}')
                self.connection.commit()
                return inserted, self.check_trails_in_db([trail['id'] for trail in trails_data])
        except Exception as e:
            print(f'Failed executing command: {command}\n Exception string:{e}')
        finally:
            self.connection.close()
    ...
    def check_trails_in_db(self, trail_wikiloc_ids):
        """
